"""Confluence OAuth install + callback routes.

Same shape as jira/routes.py — both go through the shared Atlassian OAuth
machinery and accessible-resources call — but with Confluence's own OAuth app
credentials, so a Confluence connection is independent of any Jira one.
"""
...
@router.get("/sources/confluence/install-url")
async def confluence_install_url(
    request: Request,
    ...
    if not confluence.configured():
        return {
            "configured": False,
            "error": "Confluence OAuth requires CONFLUENCE_CLIENT_ID, CONFLUENCE_CLIENT_SECRET, and CONNECTOR_SECRET_KEY.",
        }
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            "VALUES($1, $2, $3, 'confluence', $4, now() + interval '10 minutes')",
            state, current.workspace_id, current.user_id, return_to,
        )
    return {"configured": True, "url": confluence.authorize_url(state)}


@router.get("/integrations/confluence/oauth/callback")
async def confluence_oauth_callback(code: str = "", state: str = ""):
    ...
    async with pool.acquire() as conn:
        oauth_state = await conn.fetchrow(
            ...
            "WHERE state=$1 AND provider='confluence' AND consumed_at IS NULL AND expires_at > now() "
    ...
    try:
        tokens = await confluence.exchange_code(code)
        access_token = tokens["access_token"]
        resources = await confluence.accessible_resources(access_token)
    except Exception:
        redirect = (oauth_state["redirect_path"] or "/") + "?confluence=error"
    ...
    site = resources[0]
    cloud_id = site["id"]
    site_name = site.get("name") or site.get("url") or "Confluence"
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            connection_id = await conn.fetchval(
                ...
                "metadata, created_by) "
                "VALUES($1, 'confluence', $2, 'connected', $3, $4, $5, $6, $7, $8) "
            ...
            await auth.audit(conn, "confluence_install", oauth_state["workspace_id"],
            ...
            try:
                await confluence.refresh_streams(conn, connection, access_token)
            except Exception as e:  # spaces can be fetched later via Refresh
                await conn.execute(
    ...
    if resync_job_id:
        service._dispatch_sync("confluence", resync_job_id)
    redirect = (oauth_state["redirect_path"] or "/") + "?confluence=connected"
    return RedirectResponse(redirect)
